...
Copied from Willcox Github Package"""
...
from .. import errors
from ._ddt_base import DerivativeEstimatorTemplate
...
# def _method_savgol(noisy_states, dt, inputs, window_length=11, polyorder=3):
#     """Savitzky-Golay filtering"""
#     n_states, n_time = noisy_states.shape
#     smoothed = np.zeros_like(noisy_states)
...
#         smoothed[i, :] = scipy.signal.savgol_filter(noisy_states[i, :],
#                                                     window_length=window_length,
#                                                     polyorder=polyorder,
#                                                     mode='nearest')

#     _, derivatives, _ = opinf.utils.ord6(states=smoothed, dt=dt, inputs=inputs)
#     return derivatives

def _method_savgol(states, dt, inputs, window_length=11, polyorder=3):
    """
    Calculates derivatives using the Savitzky-Golay filter, which
    simultaneously smooths and differentiates the data.

    Parameters:
    -----------
    states : np.ndarray
        State data of shape (n_states, n_time).
    dt : float
        Time step size.
    inputs : np.ndarray or None
        Input data, not used in this method but kept for API consistency.
    window_length : int
    ...
    # savgol_filter can compute the derivative directly.
    # We set deriv=1 to get the first derivative.
    derivatives = scipy.signal.savgol_filter(
        states,
        window_length=window_length,
        polyorder=polyorder,
        deriv=1,
        delta=dt,
        axis=-1,  # apply filter along the time axis
        mode='nearest'
    )
    return derivatives


def _method_whittaker(states, dt, inputs, lmbd=1e2, p=3):
    """
    Whittaker-Henderson smoother using a direct sparse solver.
    This is the corrected version that fixes the ValueError.
    ...
    dt : float
        Time step size.
    lmbd : float
        The smoothing parameter lambda. Higher values mean more smoothing.
    p : int
        The order of the derivative to penalize (determines smoothness).
    """
    n_states, n_time = states.shape
    smoothed_states = np.zeros_like(states)

    # Create the identity matrix
    I = sparse.eye(n_time)

    # --- Create the difference matrix D of order p ---
    # The construction of D_p was faulty and is now corrected.

    # CORRECTED LINE: The first argument needs to be a list of lists.
    D_p = sparse.diags([[1] * (n_time - p)], [p], shape=(n_time, n_time))

    for k in range(p):
        # Binomial coefficient for the difference operator
        coeff = (-1)**(p - k) * np.math.comb(p, k)

        # CORRECTED LINE: The first argument here also needs to be a list of lists.
        D_p = D_p + sparse.diags([[coeff] * (n_time - k)], [k], shape=(n_time, n_time))
    ...
    for i in range(n_states):
        y = states[i, :]
        # Solve the sparse linear system (I + P)z = y for z
        z = spsolve(I + P, y)
        smoothed_states[i, :] = z

    # Differentiate the smoothed data
    derivatives = np.gradient(smoothed_states, dt, axis=1)
    return derivatives


def _method_ms(states, dt, inputs, m=46, n=6):
    """
    Smooths data using the Modified Sinc (MS) kernel proposed by
    Schmid et al. (2022), then computes derivatives. This method
    offers excellent stopband suppression.
    ...
    dt : float
        Time step size.
    inputs : np.ndarray or None
        Input data, not used for smoothing.
    m : int
        Half-width of the kernel.
    n : int
        Degree parameter, must be an even integer (e.g., 2, 4, 6, 8).
    ...
    np.ndarray
        Estimated derivatives.
    """
    # --- 1. Construct the Modified Sinc (MS) kernel ---
    # This implementation is based on equations 3, 4, 7, 8 from the paper.
    i = np.arange(-m, m + 1)
    x = i / (m + 1)
    alpha = 4.0

    # Window function (eq. 4)
    w = (np.exp(-alpha * x**2) +
         np.exp(-alpha * (x + 2)**2) +
         np.exp(-alpha * (x - 2)**2) -
         2 * np.exp(-alpha) - np.exp(-9 * alpha))

    # Sinc function part (from eq. 3)
    sinc_arg = ((n + 4) / 2) * np.pi * x
    sinc = np.ones_like(sinc_arg)
    # Avoid division by zero at x=0
    non_zero_mask = sinc_arg != 0
    sinc[non_zero_mask] = np.sin(sinc_arg[non_zero_mask]) / sinc_arg[non_zero_mask]

    kernel = w * sinc

    # Correction terms for flat passband (eq. 7 & 8, Table 1)
    # Simplified for n=6 as an example. For a full implementation,
    # you would need Table 1 from the paper.
    if n == 6:
        # Coefficients for n=6 from Table 1
        k0_n = 0.00172 / (0.02437 + (1.64375/m)**3)
        # Correction term from eq. 7
        correction = k0_n * np.sin((2 + 1) * np.pi * x)
        kernel += w * correction

    # Normalization (eq. 6)
    kernel /= np.sum(kernel)

    # --- 2. Apply smoothing via convolution ---
    # We use 'same' mode to keep original signal length.
    # 'reflect' is a good choice for handling boundaries.
    ...
    # --- 3. Differentiate the smoothed data ---
    # Using a simple central difference scheme for the interior points
    # and forward/backward for the boundaries. It's robust.
    derivatives = np.gradient(smoothed_states, dt, axis=1)

    return derivatives


def _method_spline_cv(noisy_states, dt, inputs, s_range=None, fast_mode=False):
    """
    Optimized Spline CV with state-wise scaling for robustness.
    Each state is scaled to [0, 1] before fitting the spline.
    """
    n_states, n_time = noisy_states.shape
    t_vec = np.arange(n_time) * dt
    derivatives = np.zeros_like(noisy_states)
    ...
    for i in range(n_states):
        ...
        # Avoid division by zero if the state is constant
        if scale < 1e-9:
            derivatives[i, :] = 0.0
            continue
        ...
        # --- Spline Fitting on Scaled Data ---
        if fast_mode:
            # Automatic s on scaled data
            spline = scipy.interpolate.UnivariateSpline(t_vec, x_scaled, s=None, k=3)
        else:
            # CV on scaled data. The s_range is now more general.
            if s_range is None:
                # A fixed, robust range for data scaled to [0, 1]
                s_range = np.logspace(-7, -2, 7)

            split = n_time // 2
            t_train, t_val = t_vec[:split], t_vec[split:]
            ...
            for s in s_range:
                try:
                    spline_cv = scipy.interpolate.UnivariateSpline(t_train, x_train, s=s, k=3)
                    error = np.mean((x_val - spline_cv(t_val))**2)
                    if error < best_error:
                        best_error = error
                        best_s = s
                except:
            ...
            # Final spline on the full, scaled data
            spline = scipy.interpolate.UnivariateSpline(t_vec, x_scaled, s=best_s, k=3)

        # --- Calculate Derivative and Unscale ---
        # Derivative of the scaled spline
        deriv_scaled = spline.derivative()(t_vec)

        # Unscale the derivative
        derivatives[i, :] = deriv_scaled * scale

    return derivatives


def estimate_derivatives(states, dt, inputs=None, method='spline_cv', **kwargs):
    """
    Unified derivative estimation with multiple methods

    Parameters:
    -----------
    states : ndarray, shape (n_states, n_time)
        State trajectories
    dt : float
        Time step size
    inputs : ndarray, optional
        Input trajectories
    method : str
        'spline_cv', 'savgol', 'moving_avg', 'ord6', 'composite'
    **kwargs : dict
    ...
    if method == 'spline_cv':
        return _method_spline_cv(states, dt, inputs, **kwargs)

    elif method == 'savgol':
        window = kwargs.get('window_length', 21)
        polyorder = kwargs.get('polyorder', 3)
        return _method_savgol(states, dt, inputs, window_length=window, polyorder=polyorder)

    elif method == 'ord6':
        # Your current baseline
        _, derivatives, _ = opinf.utils.ord6(states=states, dt=dt, inputs=inputs)
        return derivatives

    elif method == 'ms':
        return _method_ms(states, dt, inputs, m=25, n=6)

    elif method == 'whittaker':
        return _method_whittaker(states, dt, inputs, lmbd=1e2, p=3)

    elif method == 'composite':
        # Smart combination based on data characteristics
        return _composite_derivative_estimation(states, dt, inputs, **kwargs)

    else:
        raise ValueError(f"Unknown method: {method}")


def _composite_derivative_estimation(states, dt, inputs, noise_threshold=0.02):
    """
    Smart composite method that chooses based on estimated noise level
    """
    # Estimate noise level from high-frequency content
    noise_level = _estimate_noise_level(states)
    print(f"Estimated noise level: {noise_level:.4f}")

    if noise_level < 0.005:
        # Low noise: use Savitzky-Golay for accuracy
        return _method_savgol(states, dt, inputs, window_length=11)
    elif noise_level < 0.02:
        # Medium noise: wider window
        return _method_savgol(states, dt, inputs, window_length=21)
    else:
        # High noise: use robust Spline CV
        return _method_spline_cv(states, dt, inputs)


def _estimate_noise_level(states):
    """
    Simple noise level estimation from high-frequency content
    """
    ...
    for i in range(states.shape[0]):
        filtered = signal.filtfilt(b, a, states[i, :])
        noise_estimates.append(np.std(filtered))

    return np.median(noise_estimates) / np.std(states)
